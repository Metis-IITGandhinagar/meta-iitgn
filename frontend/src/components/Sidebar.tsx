...
export default function Sidebar({ isOpen, onClose, currentTier }: SidebarProps) {
  const pathname = usePathname();

  const activeTier = currentTier || "gold";
      ...
      >
        ...
        <div className="px-4 py-4 border-b border-gray-100 bg-gray-50/30 shrink-0">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-xs text-white shadow-inner transition-colors duration-300 shrink-0 ${activeTierData.progressBar}`}>
              mI 
            </div>
            <div className="flex-1 min-w-0">
              <h4 className="text-sm font-bold text-gray-800 truncate">Meta IIGN</h4>
              <p className="text-[11px] text-gray-400 truncate">meta.iitgn@iitgn.ac.in</p>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-0.5">
              ...
              >
                <User className={`h-5 w-5 transition-colors duration-200 ${
                  pathname === "/user/profile" ? "text-blue-600" : "text-gray-400 group-hover:text-gray-650"
                }`} />
              ...
              >
                <History className={`h-5 w-5 transition-colors duration-200 ${
                  pathname === "/user/contributions" ? "text-blue-600" : "text-gray-400 group-hover:text-gray-650"
                }`} />
              ...
              >
                <Settings className={`h-5 w-5 transition-colors duration-200 ${
                  pathname === "/user/settings" ? "text-blue-600" : "text-gray-400 group-hover:text-gray-650"
                }`} />
          ...
          <div className="p-3.5 rounded-xl border border-gray-150 bg-gray-50/30">
            ...
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                <span className="text-base leading-none">{activeTierData.icon}</span>
